...
There will be several files written in markdown format `example.md`. For example, the first guide in the intended order is `P1_Basics.md`. This contains very basic information about the command line interface, UNIX based operating systems, and connecting to a remote machine using `ssh`. Next in order is `P2_Slurm.md` which, predictably, discusses the SLURM job management system. Further guides added will be given a `P#_Topic.md` format name. 
