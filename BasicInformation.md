# 0. UNC Cluster Computing Workshop
...
### 3.2 The Shell
Up until now, this document has been written assuming the terminal shell is set to `bash` or `zsh`. The UNC River cluster by defualt uses the `tcsh` shell. Upon starting your `ssh` session, you can check your shell using:

```bash
jldechow@river:~$ echo $SHELL
/usr/bin/tcsh
```

To switch the shell for your current session to `bash`, simply type `bash` into the command line and hit enter. Then type `echo $0` to confirm we are now in the bash shell. The main role of the shell is the tell the system what interpreter to use to parse commands given to it.

### 3.3 Modules
Usually, the default configuration on a cluster is a main installation with a few core softwares involved, and then optional but useful softwares like `python` available to load as `modules`. On the UNC River cluster, this is not the case. I will still show an example of how to check available modules and load them on generic cluster environments

```bash
module spider                     # list available modules
module load julia/julia-1.11.7    #load julia version 1.11.7 
```